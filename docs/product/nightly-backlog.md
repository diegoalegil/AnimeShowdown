...
Ideas priorizadas que **no están en el batch activo** de ChatGPT (Round 3) ni en las épicas pendientes (#10 SSG, #11 Materialized view, etc.). Foco: cosas que mueven la aguja en retención, comunidad, moderación, SEO editorial y campañas — sin pisar el código que ChatGPT está tocando.
...
Reescribir `scripts/generate-sitemap.mjs` para usar `<image:image><image:loc>...</image:loc></image:image>` dentro del `<url>` del personaje, no como `<url>` separadas. Esto está apuntado en el Round 3 reto #10 también; si ChatGPT lo entrega esta noche, este punto se cierra automáticamente.
...
- Cosas tocadas por el batch activo de ChatGPT (PvP, comentarios, búsqueda backend, etc.).
- Épicas pendientes (#10 SSG, #11 MV stats) — esperan a que ChatGPT termine.
