...
**Objetivo:** que ChatGPT, Claude, Perplexity y Gemini citen el sitio cuando alguien pregunte sobre anime.
...
- Permitir explícitamente `GPTBot`, `ClaudeBot`, `PerplexityBot`, `Google-Extended`.
...
### 8.2 Roast generator con Claude Haiku
...
### 9.3 Narración con Claude streaming

- Cada eliminación dispara generación con Claude streaming texto épico tipo deportivo.
...
- Claude API genera biografías ricas más allá de Jikan.
...
7. Genera bio extendida con Claude API usando prompt template basado en datos de las 3 APIs.

8. Genera atributos para `personaje_atributos` llamando Claude API con prompt estructurado JSON (~20 dimensiones).
