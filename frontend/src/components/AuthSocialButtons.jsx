...
function AuthSocialButtons({ next = '/', action = 'Entrar' }) {
  const providers = [
    {
      id: 'google',
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {providers.map((provider) => (
          <a
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border bg-bg px-4 py-2.5 text-sm font-semibold text-fg-strong transition-colors hover:border-accent/60 hover:text-accent focus:outline-none focus:ring-2 focus:ring-accent/40"
          >
            {provider.icon}
            {provider.label}
          </a>
