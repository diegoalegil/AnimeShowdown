/* eslint-disable */
/**
 * ChatGPT Image Downloader — runtime servido a animeshowdown.dev/tools/
 *
 * Se carga via bookmarklet (ver docs/scripts/chatgpt-image-downloader.md)
 * en el contexto de chatgpt.com / chat.openai.com. NO se sube a ningún
...
;(function () {
  ...
  // ChatGPT renderiza las imágenes generadas en <img> dentro del DOM
  // de la conversación. Filtramos por hosts de OpenAI conocidos para
  ...
  if (imagenes.length === 0) {
    alert(
      'No detecté imágenes generadas en esta conversación.\n' +
        'Si ChatGPT está aún generándolas, espera a que terminen y vuelve a pulsar.',
    )
    return
  }
  ...
  const propuestas = imagenes.map(
    (_, i) => nombresGuion[i] || `chatgpt-${String(i + 1).padStart(2, '0')}`,
  )
  ...
  panel.querySelector('#__as_download').addEventListener('click', async () => {
    ...
    for (let i = 0; i < imagenes.length; i++) {
      ...
      const nombre = (inputs[i]?.value || `chatgpt-${i + 1}`).replace(/\s+/g, '-').toLowerCase()
      try {
