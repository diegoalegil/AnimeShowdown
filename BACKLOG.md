...
**Si encuentras un PR ya hecho** (Codex previo, sesión anterior, contribución externa): márcalo como DONE en `PROGRESS.md` con el commit hash existente y salta al siguiente.
...
- Buscar rastros públicos de proceso automatizado en archivos `*.md` y comentarios de código: ChatGPT, OpenAI, Claude, Codex, "generated by", "Co-authored-by".
...
**Decisiones pre-tomadas:**
- Borrar del repo (si existen): `HANDOFF_SESSION_*.md`, `MERGE_REPORT_*.md`, `AUDIT*.md`, `*_AUDIT*.md`, `CODEX*.md`, archivos de prompts guardados, notas internas.
- Añadir a `.gitignore`: `AUDIT*.md`, `*_AUDIT*.md`, `CODEX*.md`, `MERGE_REPORT*.md`, `HANDOFF_SESSION_*.md`, `.codex/`, `ai-notes/`, `private-notes/`, `prompt-notes/`.
- Limpiar de comentarios de código cualquier mención a herramientas automatizadas. Mantener comentarios técnicos legítimos.

**Tests:** `git status` limpio. `grep -ri "co-authored-by\|generated by\|chatgpt\|openai\|claude\|codex" --include='*.md' --include='*.jsx' --include='*.js' --include='*.java' .` devuelve solo coincidencias técnicas legítimas (si alguna).
...
**Estado:** parcial — Codex hizo home hierarchy + vote arena + character compact media. Quedan items concretos del audit visual pendientes.
...
**Decisiones:** este PR depende del MANUAL TRACK de imágenes (humano genera los assets primero). Codex solo implementa la integración cuando los archivos ya estén en `frontend/public/assets/` o `frontend/img/`.
